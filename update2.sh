...
EOF
cat << 'EOF' > /opt/farmwatch/backend/routers/farms.py
...
from models.models import Farm, User, LineGroup, Message, Photo
from schemas import FarmCreate, FarmResponse, LineGroupLink, LineGroupResponse
from deps import get_db, get_current_user, require_role
...
async def link_group(
    ...
    # 綁定 LINE 群組與農場 (Link LINE group to farm)
...
@router.get("/unbound-groups", response_model=List[LineGroupResponse])
async def list_unbound_groups(
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(require_role(["boss", "supervisor"]))
):
    # 獲取未綁定的 LINE 群組 (Get unbound LINE groups)
    result = await db.execute(select(LineGroup).where(LineGroup.farm_id == None))
    return result.scalars().all()
  ...
  try {
    ...
    container.innerHTML = users.map(u => `
      <div style="padding: 1rem; border-bottom: 1px solid var(--border-color);">
        ...
        <div style="display: flex; gap: 0.5rem;">
          <input type="password" id="pw-reset-${u.id}" placeholder="新密码 (New Password)" class="form-input" style="flex: 1; padding: 0.5rem; border-radius: var(--radius-sm); border: 1px solid rgba(0,0,0,0.1);">
          <button class="btn btn-secondary reset-pw-btn" data-id="${u.id}" style="padding: 0.5rem 1rem; border-radius: var(--radius-sm); cursor: pointer;">
            重设 (Reset)
          </button>
        </div>
      </div>
    ...
    document.querySelectorAll('.reset-pw-btn').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        ...
        const newPw = document.getElementById(`pw-reset-${userId}`).value;
        if (!newPw) {
          showToast('❌ 请输入新密码 (Enter new password)', 'error');
          return;
        }
        ...
        try {
          ...
          showToast('✅ 密码已重设 (Password reset)', 'success');
          document.getElementById(`pw-reset-${userId}`).value = '';
        } catch(err) {
          showToast('❌ 重设失败 (Failed)', 'error');
        } finally {
          btn.disabled = false;
        }
      });
