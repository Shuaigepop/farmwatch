...
EOF
cat << 'EOF' > /opt/farmwatch/backend/services/ai_service.py
import json
import asyncio
import time
...
class AIService:
    ...
    def _analyze_image_sync(self, image_path: str) -> str:
        ...
        prompt = """
        請分析這張農作物/農場的照片。
        請回傳一個包含以下鍵值的 JSON 字串：
        - "status": 健康狀態，必須是 "healthy", "warning", 或 "critical" 之一
        - "notes": 詳細觀察筆記與建議（使用繁體中文）
        - "confidence": 信心分數 (0.0 到 1.0)
        只回傳 JSON，不要其他文字。
        """
        def _call():
    ...
    def _generate_daily_summary_sync(self, date_str: str, messages: list, tasks: list, photos: list) -> str:
        prompt = f"""
        請根據以下提供的農場紀錄，生成一份繁體中文的每日農場總結報告，日期為 {date_str}。
    ...
    async def generate_daily_summary(self, date_str: str, messages: list, tasks: list, photos: list) -> str:
        return await asyncio.to_thread(self._generate_daily_summary_sync, date_str, messages, tasks, photos)
